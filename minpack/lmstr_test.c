/* lmstr_test.f -- translated by f2c (version 20160102).
...
/* Main program */ int MAIN__(void)
{
    ...
    static char fmt_80[] = "(\0021SUMMARY OF \002,i3,\002 CALLS TO LMSTR1"
	    "\002/)";
...
L10:
	...
	refnum_1.nfev = 0;
	refnum_1.njev = 0;
	lmstr1_((U_fp)fcn_, &m, &n, x, fvec, fjac, &ldfjac, &tol, &info, iwa, 
		wa, &lwa);
	ssqfcn_(&m, &n, x, fvec, &refnum_1.nprob);
...
L30:
    ...
    e_wsfe();
    io___34.ciunit = nwrite;
    s_wsfe(&io___34);
    e_wsfe();
    i__1 = ic;
    for (i__ = 1; i__ <= i__1; ++i__) {
	io___35.ciunit = nwrite;
	s_wsfe(&io___35);
...
/* Subroutine */ int fcn_(integer *m, integer *n, doublereal *x, doublereal *
	fvec, doublereal *fjrow, integer *iflag)
{
    /* System generated locals */
    integer i__1;
    ...
    /* Local variables */
    static integer j;
    static doublereal temp[2600]	/* was [65][40] */;
    extern /* Subroutine */ int ssqjac_(integer *, integer *, doublereal *, 
	    doublereal *, integer *, integer *), ssqfcn_(integer *, integer *,
...
/*     ********** */
    ...
    if (*iflag == 2) {
	ssqjac_(m, n, &x[1], temp, &c__65, &refnum_1.nprob);
    }
    if (*iflag == 1) {
	++refnum_1.nfev;
    }
...
/* Subroutine */ int ssqjac_(integer *m, integer *n, doublereal *x, 
	doublereal *fjac, integer *ldfjac, integer *nprob)
{
    /* Initialized data */
    ...
    static doublereal c20 = 20.;
    static doublereal c29 = 29.;
    static doublereal c45 = 45.;
    static doublereal c100 = 100.;
    ...
    /* Builtin functions */
    double atan(doublereal), sqrt(doublereal), exp(doublereal), sin(
	    doublereal), cos(doublereal);

    /* Local variables */
    static integer i__, j, k;
    static doublereal s2, dx, ti;
    static integer mm1, nm1;
    static doublereal div, tpi, tmp1, tmp2, tmp3, tmp4, prod, temp;
...
L10:
    temp = two / (doublereal) (*m);
    i__1 = *n;
    for (j = 1; j <= i__1; ++j) {
	i__2 = *m;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    fjac[i__ + j * fjac_dim1] = -temp;
/* L20: */
	}
	fjac[j + j * fjac_dim1] += one;
/* L30: */
    }
    goto L500;
...
L40:
	...
	i__2 = *m;
	for (i__ = 1; i__ <= i__2; ++i__) {
	    fjac[i__ + j * fjac_dim1] = (doublereal) i__ * (doublereal) j;
/* L50: */
	}
/* L60: */
    }
...
L130:
    fjac[fjac_dim1 + 1] = -c20 * x[1];
    fjac[(fjac_dim1 << 1) + 1] = ten;
    fjac[fjac_dim1 + 2] = -one;
    fjac[(fjac_dim1 << 1) + 2] = zero;
    goto L500;
...
/* Computing 2nd power */
    ...
    tmp1 = tpi * temp;
    tmp2 = sqrt(temp);
    fjac[fjac_dim1 + 1] = c100 * x[2] / tmp1;
    fjac[(fjac_dim1 << 1) + 1] = -c100 * x[1] / tmp1;
    fjac[fjac_dim1 * 3 + 1] = ten;
    fjac[fjac_dim1 + 2] = ten * x[1] / tmp2;
    fjac[(fjac_dim1 << 1) + 2] = ten * x[2] / tmp2;
...
L180:
    fjac[fjac_dim1 + 1] = one;
    fjac[(fjac_dim1 << 1) + 1] = x[2] * (ten - three * x[2]) - two;
    fjac[fjac_dim1 + 2] = one;
    fjac[(fjac_dim1 << 1) + 2] = x[2] * (two + three * x[2]) - c14;
    goto L500;
...
/* Computing 2nd power */
	...
	tmp4 = d__1 * d__1;
	fjac[i__ + fjac_dim1] = -one;
	fjac[i__ + (fjac_dim1 << 1)] = tmp1 * tmp2 / tmp4;
	fjac[i__ + fjac_dim1 * 3] = tmp1 * tmp3 / tmp4;
/* L200: */
    }
    goto L500;
...
L210:
	...
	fjac[i__ + fjac_dim1] = -tmp1 / tmp2;
	fjac[i__ + (fjac_dim1 << 1)] = -v[i__ - 1] * x[1] / tmp2;
	fjac[i__ + fjac_dim1 * 3] = fjac[i__ + fjac_dim1] * fjac[i__ + (
		fjac_dim1 << 1)];
	fjac[i__ + (fjac_dim1 << 2)] = fjac[i__ + fjac_dim1 * 3] / v[i__ - 1];
/* L220: */
    }
    goto L500;
...
L230:
	...
	tmp1 = x[2] / temp;
	tmp2 = exp(tmp1);
	fjac[i__ + fjac_dim1] = tmp2;
	fjac[i__ + (fjac_dim1 << 1)] = x[1] * tmp2 / temp;
	fjac[i__ + fjac_dim1 * 3] = -tmp1 * fjac[i__ + (fjac_dim1 << 1)];
/* L240: */
    }
    goto L500;
...
/* L260: */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = j - 1;
	    fjac[i__ + j * fjac_dim1] = dx * ((doublereal) i__2 - temp);
	    dx = div * dx;
/* L270: */
	}
/* L280: */
...
/* L300: */
    ...
    fjac[fjac_dim1 + 30] = one;
    fjac[fjac_dim1 + 31] = -two * x[1];
    fjac[(fjac_dim1 << 1) + 31] = one;
    goto L500;
...
L310:
	...
	temp = (doublereal) i__;
	tmp1 = temp / ten;
	fjac[i__ + fjac_dim1] = -tmp1 * exp(-tmp1 * x[1]);
	fjac[i__ + (fjac_dim1 << 1)] = tmp1 * exp(-tmp1 * x[2]);
	fjac[i__ + fjac_dim1 * 3] = exp(-temp) - exp(-tmp1);
/* L320: */
    }
    goto L500;
...
L330:
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	temp = (doublereal) i__;
	fjac[i__ + fjac_dim1] = -temp * exp(temp * x[1]);
	fjac[i__ + (fjac_dim1 << 1)] = -temp * exp(temp * x[2]);
/* L340: */
    }
    goto L500;
...
L350:
	...
	fjac[i__ + fjac_dim1] = two * tmp1;
	fjac[i__ + (fjac_dim1 << 1)] = temp * fjac[i__ + fjac_dim1];
	fjac[i__ + fjac_dim1 * 3] = two * tmp2;
	fjac[i__ + (fjac_dim1 << 2)] = ti * fjac[i__ + fjac_dim1 * 3];
/* L360: */
    }
    goto L500;
...
L460:
	...
	tmp2 = exp(-x[5] * temp);
	fjac[i__ + fjac_dim1] = -one;
	fjac[i__ + (fjac_dim1 << 1)] = -tmp1;
	fjac[i__ + fjac_dim1 * 3] = -tmp2;
	fjac[i__ + (fjac_dim1 << 2)] = temp * x[2] * tmp1;
	fjac[i__ + fjac_dim1 * 5] = temp * x[3] * tmp2;
/* L470: */
    }
    goto L500;
...
/* Computing 2nd power */
	...
	fjac[i__ + fjac_dim1] = -tmp1;
	fjac[i__ + (fjac_dim1 << 1)] = -tmp2;
	fjac[i__ + fjac_dim1 * 3] = -tmp3;
	fjac[i__ + (fjac_dim1 << 2)] = -tmp4;
	fjac[i__ + fjac_dim1 * 5] = temp * x[1] * tmp1;
/* Computing 2nd power */
	d__1 = temp - x[9];
	fjac[i__ + fjac_dim1 * 6] = x[2] * (d__1 * d__1) * tmp2;
/* Computing 2nd power */
	d__1 = temp - x[10];
	fjac[i__ + fjac_dim1 * 7] = x[3] * (d__1 * d__1) * tmp3;
/* Computing 2nd power */
	d__1 = temp - x[11];
	fjac[i__ + (fjac_dim1 << 3)] = x[4] * (d__1 * d__1) * tmp4;
	fjac[i__ + fjac_dim1 * 9] = -two * x[2] * x[6] * (temp - x[9]) * tmp2;
	fjac[i__ + fjac_dim1 * 10] = -two * x[3] * x[7] * (temp - x[10]) * 
		tmp3;
	fjac[i__ + fjac_dim1 * 11] = -two * x[4] * x[8] * (temp - x[11]) * 
		tmp4;
/* L490: */
    }
L500:
